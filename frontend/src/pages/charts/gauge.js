...
class GaugeGraph extends React.Component {
  ...
  renderText(amount, total) {
    ...
    amountText.style('opacity', 0).transition().duration(1000).delay(1500).style('opacity', 1).text(`${amount}%`)

    totalText.style('opacity', 0).transition().duration(1000).delay(1700).style('opacity', 1).text(`/${total}`)
  }
